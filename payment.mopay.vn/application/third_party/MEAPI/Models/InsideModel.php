...
class InsideModel extends CI_Model {
    ...
    public function get_items($params) {
        ...
        $result = StoreProcedure::call('sp_items_get', $params, $this->_db_slave);
        return is_object($result) ? $result->result_array() : FALSE;
    }
    
    public function select_items($params) {
        if (empty($this->_db_slave) == TRUE) {
            $this->_db_slave = $this->load->database(array('db' => 'payment_info', 'type' => 'slave'), TRUE);
        }
        $result = StoreProcedure::call('sp_items_info', $params, $this->_db_slave);
        return is_object($result) ? $result->row_array() : FALSE;
    }

    public function insert_items($params) {
        if (empty($this->_db) == TRUE) {
            $this->_db = $this->load->database(array('db' => 'payment_info', 'type' => 'master'), TRUE);
        }
        $result = StoreProcedure::call('sp_items_insert', $params, $this->_db);
        if (is_object($result)) {
    ...
    public function update_items($params) {
        ...
        $result = StoreProcedure::call('sp_items_update', $params, $this->_db);
        if (is_object($result)) {
            $data = $result->row_array();
            if ($data['result'])
                return TRUE;
    ...
    public function delete_items($params) {
        ...
        $result = StoreProcedure::call('sp_items_del', $params, $this->_db);
        if (is_object($result)) {
            $data = $result->row_array();
            if ($data['result'])
                return TRUE;
    ...
    public function get_connections($params) {
        ...
        $result = StoreProcedure::call('sp_connections_get', $params, $this->_db_slave);        
        return is_object($result) ? $result->result_array() : FALSE;
    }
    
    public function get_apps($params) {
        if (empty($this->_db_slave) == TRUE) {
            $this->_db_slave = $this->load->database(array('db' => 'payment_info', 'type' => 'slave'), TRUE);
        }
        $result = StoreProcedure::call('sp_app_get', $params, $this->_db_slave);
        return is_object($result) ? $result->result_array() : FALSE;
    }
    ...
    public function update_app($params) {
        ...
        $result = StoreProcedure::call('sp_app_update', $params, $this->_db);
        if (is_object($result)) {
            $data = $result->row_array();
            if ($data['result'])
                return TRUE;
    ...
    public function insert_app($params) {
        ...
        $result = StoreProcedure::call('sp_app_insert', $params, $this->_db);
        if (is_object($result)) {
            $data = $result->row_array();            
            if ($data['result'])
                return TRUE;
    ...
    public function get_paymentdata(array $keys, array $fields = array(), $page = 0, $limit = 0) {
        if (empty($this->_db_slave) == TRUE) {
            $this->_db_slave = $this->load->database(array('db' => 'payment_info', 'type' => 'slave'), TRUE);
        }
        $this->_db_slave->select(count($fields) == 0 ? '*' : implode(',', $fields));
        foreach ($keys as $key => $value) {
            if (is_array($value))
                $this->_db_slave->where_in($key, $value);
            else
                $this->_db_slave->where($key, $value);
        }

        if ($limit != 0) {
            $this->_db_slave->limit($limit,$page)->order_by("id", "asc");
        }

        $query = $this->_db_slave->get("report_daily_payment");
    ...
    public function get_where(array $keys, array $fields = array(),$table) {
        if (empty($this->_db_slave) == TRUE) {
            $this->_db_slave = $this->load->database(array('db' => 'payment_info', 'type' => 'slave'), TRUE);
        }
        $this->_db_slave->select(count($fields) == 0 ? '*' : implode(',', $fields));
        ...
        $query = $this->_db_slave->get($table);
        //echo $this->getConnection()->last_query(); die;
        $queryResult = ($query != FALSE) ? $query->result_array() : FALSE;
    ...
    public function getGroup(array $keys, array $fields = array(),array $group,$table) {
        if (empty($this->_db_slave) == TRUE) {
            $this->_db_slave = $this->load->database(array('db' => 'payment_info', 'type' => 'slave'), TRUE);
        }
        $this->_db_slave->select(count($fields) == 0 ? '*' : implode(',', $fields));
